...
export async function obtenerResumenDashboard(req, res) {
  try {
    ...
    const montoTotal = ventas.reduce((sum, venta) => sum + (venta.total_pagar || 0), 0)
    ...
    const montoVentasHoy = ventasHoy.reduce((sum, venta) => sum + (venta.total_pagar || 0), 0)

    // Contar métodos de pago
    const ventasEfectivo = ventas.filter((venta) => venta.metodo_pago === "efectivo").length
    const ventasTransferencia = ventas.filter((venta) => venta.metodo_pago === "transferencia").length
...
export async function obtenerVentasRecientes(req, res) {
  try {
    ...
    const ventasOrdenadas = [...ventas].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
  ...
  try {
    ...
    if (periodo === "diario") {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const montoDelDia = ventasDelDia.reduce((sum, venta) => sum + (venta.total_pagar || 0), 0)
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        inicioSemana.setDate(inicioSemana.getDate() - (inicioSemana.getDay() + 7 * i))
        ...
        const montoDeLaSemana = ventasDeLaSemana.reduce((sum, venta) => sum + (venta.total_pagar || 0), 0)
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const montoDelMes = ventasDelMes.reduce((sum, venta) => sum + (venta.total_pagar || 0), 0)
    ...
    console.error(`Error al obtener estadísticas ${req.params.periodo}:`, error)
    res.status(500).json({
      exito: false,
